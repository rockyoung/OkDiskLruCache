 ...
 * and a fixed number of values. Each key must match the regex `[a-z0-9_-]{1,120}`. Values are byte
 ...
 */
class OkDiskLruCache internal constructor(
  ...
  private val cleanupRunnable = Runnable {
    synchronized(this@OkDiskLruCache) {
  ...
  fun initialize() {
    ...
    if (fileSystem.exists(journalFile)) {
      try {
        ...
        println("DiskLruCache $directory is corrupt: ${journalIsCorrupt
          .message}, removing")
      }
  ...
  private fun newJournalWriter(): BufferedSink {
    ...
    val faultHidingSink = FaultHidingSink(fileSink) {
      assert(Thread.holdsLock(this@OkDiskLruCache))
  ...
  private fun readJournalLine(line: String) {
    ...
    when {
      secondSpace != -1
        ...
        val parts = line.substring(secondSpace + 1).split(SPACE_CHAR)
        entry.readable = true
  ...
  internal fun rebuildJournal() {
    ...
    fileSystem.sink(journalFileTmp).buffer().use { sink ->
      sink.writeUtf8(MAGIC).writeByte(BREAK_INT)
      sink.writeUtf8(VERSION_1).writeByte(BREAK_INT)
      sink.writeDecimalLong(appVersion.toLong()).writeByte(BREAK_INT)
      sink.writeDecimalLong(valueCount.toLong()).writeByte(BREAK_INT)
      sink.writeByte(BREAK_INT)

      for (entry in lruEntries.values) {
        if (entry.currentEditor != null) {
          sink.writeUtf8(DIRTY).writeByte(SPACE_INT)
          sink.writeUtf8(entry.key)
          sink.writeByte(BREAK_INT)
        } else {
          sink.writeUtf8(CLEAN).writeByte(SPACE_INT)
          sink.writeUtf8(entry.key)
  ...
  /**
   * Returns a snapshot of the entry named [key], will return null if:
   * (1) it doesn't exist or (2) it is not currently readable.
  ...
  @Synchronized @Throws(IOException::class)
  @JvmOverloads
  fun edit(key: String, expectedSequenceNumber: Long = ANY_SEQUENCE_NUMBER
  ): Editor? {
    initialize()
  ...
  internal fun completeEdit(editor: Editor, success: Boolean) {
    ...
    journalWriter?.apply {
      if (entry.readable || success) {
        entry.readable = true
        writeUtf8(CLEAN).writeByte(SPACE_INT)
        writeUtf8(entry.key)
        ...
        writeUtf8(REMOVE).writeByte(SPACE_INT)
        writeUtf8(entry.key)
        writeByte(BREAK_INT)
      }
      flush()
  ...
  private fun journalRebuildRequired(): Boolean {
    return redundantOpCount >= 2048 && redundantOpCount >= lruEntries.size
  }
  ...
  @Synchronized @Throws(IllegalArgumentException::class)
  private fun validateKey(key: String) {
    require(LEGAL_KEY_REGEX.matches(key)) {
  ...
  private fun validateIndex(index: Int) {
    require(index in 0 until valueCount) {
      "Expected index is in [0, $valueCount), but $index was input in!"
    }
  }
  ...
  fun snapshots(): MutableIterator<Snapshot> {
    ...
    return object : MutableIterator<Snapshot> {
      ...
      override fun remove() {
        ...
        try {
          this@OkDiskLruCache.remove(removeSnapshot.key())
        } catch (_: IOException) {
  ...
  inner class Snapshot internal constructor(
     ...
     */
    @Throws(IOException::class)
    fun edit(): Editor? = this@OkDiskLruCache.edit(key, sequenceNumber)

    /** Returns the unbuffered stream with the value for [index]. */
    fun getSource(index: Int): Source = sources[index]

    /** Returns the clean cache file for [index]. */
  ...
  internal inner class Entry internal constructor(
    ...
    internal fun writeLengths(writer: BufferedSink) {
      for (length in lengths) {
        writer.writeByte(SPACE_INT).writeDecimalLong(length)
      }
    }

    @Throws(IOException::class)
    private fun invalidLengths(strings: List<String>): IOException {
      throw IOException("unexpected length part: ${strings
        .joinToString(SPACE_CHAR.toString())}, valueCount=$valueCount")
    }
    ...
    internal fun snapshot(): Snapshot? {
      ...
      try {
        ...
        return Snapshot(key, sequenceNumber, sources,
          cleanFiles.toList(), lengths)
      } catch (_: FileNotFoundException) {
  ...
  companion object {
    ...
    private const val SPACE_INT = SPACE_CHAR.toInt()
    private const val BREAK_INT = '\n'.toInt()
    ...
    internal const val ANY_SEQUENCE_NUMBER: Long = -1
    internal const val STRING_KEY_PATTERN = "[a-z0-9_-]{1,120}"

    @JvmField val LEGAL_KEY_REGEX = STRING_KEY_PATTERN.toRegex()
     ...
     * Default is `64` Mb.
     * @param fileSystem the [FileSystem] implementation. Default is [FileSystem.SYSTEM]
     */
    fun create(
      directory: File,
      appVersion: Int,
      valueCount: Int = 1,
      maxSize: Long = 64 * 1024 * 1024,
      ...
      val executor = ThreadPoolExecutor(
        ...
        TimeUnit.SECONDS,
        LinkedBlockingQueue(),
        threadFactory("OkDiskLruCache", true)
      )

      return OkDiskLruCache(fileSystem, directory, appVersion,
        valueCount, maxSize, executor)
    }
  }
